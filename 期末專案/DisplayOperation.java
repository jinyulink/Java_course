...
public class DisplayOperation implements IOperation {
	public void work(BookList booklist) {
        ...
        for (int i = 0; i < booklist.getBooks().size(); i++) {
            ...
            info+=i+1+". 書名: "+book.getName(); //jin
			info+="\n>> 作者: "+book.getAuthor();
			info+="\n>> 出版社: "+book.getPublish();
			info+="\n>> 型態: "+book.getType(); //jin
            info+="\n>> 是否被借閱: "+(book.isBorrowed()?"已借出":"尚未借出"); //jin
            info+="\n>> 借閱次數: "+book.getBorrow();  //jin
            info+="\n>> 搜尋次數: "+book.getSearch();  //jin
			info+="\n\n==========================\n\n";
	    ...
	    int number = JOptionPane.showConfirmDialog(null, scrollPane, "管理員介面",JOptionPane.DEFAULT_OPTION,0,icon);
